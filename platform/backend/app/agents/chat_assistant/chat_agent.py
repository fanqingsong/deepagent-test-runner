...
class ChatAgent:
    ...
    async def _ensure_store(self):
        """Lazily initialize the PostgreSQL checkpointer and store."""
        if self._ready:
            return
        ...
        # Run sync PG setup in a thread to avoid blockbuster blocking detection
        def _init_sync():
            stack = ExitStack()
            stack.__enter__()
            ...
            store = stack.enter_context(
                PostgresStore.from_conn_string(conn_string)
            )
            store.setup()
        ...
        self._agent = create_deep_agent(
            ...
            subagents=[
                ...
                get_sandbox_data_analysis_subagent(llm),
        ...
        logger.info("Chat agent initialized with PostgreSQL checkpointer and store")
