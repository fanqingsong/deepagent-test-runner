...
async def send_message(
    ...
    updated_plan = None
    try:
        from app.agents.deepagents_test_runner.planner_agent import generate_test_plan, refine_test_plan
...
async def respond_to_failure(
    ...
    if request.action == "retry":
        ...
        if test_run and test_run.test_definition_id:
            ...
            modified_plan["test_definition_id"] = str(test_run.test_definition_id)

            from app.temporal import get_temporal_client
            from app.temporal.workflows.test_execution import RetryTestWorkflow

            client = await get_temporal_client()
            await client.start_workflow(
                RetryTestWorkflow.run,
                args=[run_id, modified_plan, environment],
                id=f"retry-test-{run_id}",
