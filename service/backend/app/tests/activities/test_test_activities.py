...
async def test_prepare_test_success():
    ...
    with patch('app.activities.test_activities.run_with_session') as mock_run_session:
...
async def test_mark_run_failed():
    ...
    # Mock run_with_session
    with patch('app.activities.test_activities.run_with_session') as mock_run_session:
        with patch('app.activities.test_activities.ExecutionService', return_value=mock_service):
            async def mock_session_func(db_func):
